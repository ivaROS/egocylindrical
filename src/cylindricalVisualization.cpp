...
CylindricalVisualization::CylindricalVisualization() :it_(nh_), propagator_(nh_)
{
    ...
    message_filters::Subscriber<sensor_msgs::Image> depthSub(nh_, "/camera/depth/image_raw", 20);
    message_filters::Subscriber<sensor_msgs::CameraInfo> depthInfoSub(nh_, "/camera/depth/camera_info", 20);
    ...
    message_filters::TimeSynchronizer<sensor_msgs::Image, sensor_msgs::CameraInfo> timeSynchronizer(depthSub, tf_filter, 20);
